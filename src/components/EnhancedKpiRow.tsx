import svgPaths from "../imports/svg-jf2bbr4t64";
...
function ButtonContent({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="content-stretch flex gap-[8px] items-center justify-center relative rounded-[8px] shrink-0 cursor-pointer hover:opacity-80 transition-opacity"
...
function InfoOrCta({ onClick }: { onClick: () => void }) {
  return (
...
function KpiCardBase({ onClick }: { onClick: () => void }) {
  return (
...
function KpiCard({ onClick }: { onClick: () => void }) {
  return (
...
export function EnhancedKpiRow({ onAlertasClick }: { onAlertasClick?: () => void }) {
  return (
    <div className="w-full overflow-x-auto">
      <div className="min-w-[1000px]">
        <KpiRow onAlertasClick={onAlertasClick || (() => {})} />
      </div>
    </div>
  );
}
